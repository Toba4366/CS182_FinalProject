"""
Entry point for training models on 5 states, 8 actions FSMs (harder than baseline).

Required demo length for 99.5% accuracy: ~70 demos
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train on 5 states, 8 actions FSM")
    ...
    parser.add_argument("--demo-length", type=int, default=70)
...
def main():
    ...
    # FSM configuration: 5 states, 8 actions
    ...
    dataset_cfg = ICLDatasetConfig(
        ...
        cache_path=Path("data/icl_dataset_5s8a.pt"),
    )
    ...
    print(f"\n5s8a {args.model}: {model_cfg.num_layers} layers, {model_cfg.d_model} d_model")
    ...
    checkpoint_path = log_dir / f"5s8a_{args.model}_{timestamp}.pt"
    torch.save({
        "epoch": args.epochs,
    ...
    metrics = {
        "experiment": f"5s8a_{args.model}",
        "timestamp": timestamp,
        "fsm_config": {"num_states": 5, "num_actions": 8},
        "model_config": model_cfg.__dict__,
        "training_config": {
            "batch_size": trainer_cfg.batch_size,
            "learning_rate": trainer_cfg.learning_rate,
    ...
    metrics_path = log_dir / f"5s8a_{args.model}_{timestamp}_metrics.json"
