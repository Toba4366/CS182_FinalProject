"""
GRU Comparison Experiment
=========================

This script trains a GRU model on the ICL FSM task to fill the gap between
vanilla RNN and LSTM in the architecture spectrum.

Research Questions:
- Does simple gating (GRU) help more than raw capacity (large RNN)?
- Where does GRU fall on the performance spectrum (RNN < GRU < LSTM)?
- Is GRU a "sweet spot" for efficiency vs performance?
"""
...
from src.models.moore_gru import create_moore_gru
from src.training.gru_trainer import MooreGRUTrainer, TrainingConfig, ICLDataCollator


def parse_args():
    parser = argparse.ArgumentParser(description="GRU for ICL FSM Task")
    ...
    parser.add_argument("--experiment-name", type=str, default="gru_baseline",
...
def main():
    ...
    print(f"  vs Vanilla RNN (256d, 2L): {param_count / 200_000:.2f}x")
    print(f"  vs LSTM (256d, 2L): {param_count / 800_000:.2f}x")
    ...
    # Create trainer
    test_loader_for_trainer = DataLoader(
        test_dataset,
        batch_size=args.batch_size,
        shuffle=False,
        collate_fn=collator,
    ...
    trainer = MooreGRUTrainer(
        model=model,
        train_loader=train_loader,
        val_loader=val_loader,
        test_loader=test_loader_for_trainer,
