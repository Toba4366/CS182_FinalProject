"""
Small model experiments with d_model=64 to match transformer parameter counts.

Fair comparison: All models should have similar parameter counts (~50-100K params).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Small model comparison (d_model=64)")
    parser.add_argument("--model", type=str, default="lstm",
                       choices=["lstm", "vanilla_rnn", "deep_rnn"])
    parser.add_argument("--num-samples", type=int, default=10000)
    parser.add_argument("--epochs", type=int, default=50)
    parser.add_argument("--batch-size", type=int, default=8)
    parser.add_argument("--d-model", type=int, default=64,
                       help="Hidden dimension (default 64 for fair comparison)")
...
def main():
    ...
    if args.model == "lstm":
        ...
        # For fair comparison with d_model=64, use fewer layers
        num_layers = args.num_layers or 4  # ~4 layers ≈ similar params to 2-layer LSTM
    ...
    print(f"\nSmall {args.model}: d_model={args.d_model}, {model_cfg.num_layers} layers")
    ...
    checkpoint_path = log_dir / f"small_{args.model}_d{args.d_model}_{timestamp}.pt"
    torch.save({
        "epoch": args.epochs,
    ...
    metrics = {
        "experiment": f"small_{args.model}_d{args.d_model}",
        "timestamp": timestamp,
        "model_config": model_cfg.__dict__,
        "training_config": {
            "batch_size": trainer_cfg.batch_size,
    ...
    metrics_path = log_dir / f"small_{args.model}_d{args.d_model}_{timestamp}_metrics.json"
