"""
LSTM for predicting Moore machine states.
...
class MooreLSTM(nn.Module):
    """An LSTM specialised for Moore machine state prediction."""
    ...
    def forward(
        ...
        # LSTM forward pass
        lstm_output, _ = self.lstm(x)  # (B, T, d_model * num_directions)
        lstm_output = self.dropout(lstm_output)
        
        # Apply output head
        logits = self.head(lstm_output)  # (B, T, num_states)
...
def create_moore_lstm(
    ...
    """Factory function for creating a Moore LSTM."""
    config = LSTMConfig(
